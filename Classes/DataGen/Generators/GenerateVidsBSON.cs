...
{
    public class GenerateVidsBSON : IDataGenerator<VidsBSON>
    {
        private static int[] randHeights, randWidths;

        public GenerateVidsBSON()
        {
        ...
        public List<VidsBSON> GenerateData(int count)
        {
            if (count <= 1000)
        ...
        [Time("GenerateVidsMongo.SingleThread({count})")]
        protected List<VidsBSON> SingleThread(int count)
        {
            RandomDataGenerator randomGen = new RandomDataGenerator();
            Random rand = new Random();

            List<VidsBSON> dataList = new List<VidsBSON>();
            for (int i = 0; i < count; i++)
            {
                VidsBSON v = new VidsBSON
                {
                    // _id = ObjectId.GenerateNewId(), // this is auto-generated by MongoDB driver if none provided
                    filename = randomGen.RandomString(rand.Next(10, 45)) + ".mp4",
                    filesizebyte = randomGen.RandomLong(5000, 20000000000), // max 20 GB
            ...
            Parallel.For(0, partitions, i =>
            {
                ...
                for (int j = start; j < end; j++)
                {
                    var v = new VidsBSON
                    {
                        ...
                        filesizebyte = localRandomGen.RandomLong(5000, 20000000000), // max 20 GB
